...
import { claritySchema, truthSchema, alignmentSchema, decisionFormSchema } from "../../_lib/schemas";
import type { DecisionFormData, CalmCheckData } from "../../_lib/schemas";
import { createDecision, updateDecision } from "../../_lib/actions";
...
export function DecisionWizard({ initialData, editId }: DecisionWizardProps) {
  ...
  const handleSubmit = (calmCheck?: CalmCheckData) => {
    setIsSubmitting(true);
    try {
      const data = form.getValues();
      let decision;
      if (editId) {
        decision = updateDecision(editId, data, calmCheck);
      } else {
        decision = createDecision(data, calmCheck);
      }
      router.push(`/decision-os/decisions/${decision.id}`);
    } catch (error) {
